...
generated by the separate remote FastEmbed miniCOIL service using `Qdrant/minicoil-v1`; the
collection config uses the `IDF` modifier required by miniCOIL. The same metadata-enriched
`vectorized_content` is supplied to both vectorizers. During retrieval, Qdrant executes filtered
dense and sparse prefetches and applies native reciprocal-rank fusion (RRF), returning up to
`HYBRID_CANDIDATE_LIMIT` candidates for parent collapse and Jina reranking.
